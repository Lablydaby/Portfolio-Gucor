...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '', label, onPaperClick }: FolderProps) => {
  const maxItems = 3;
  ...
  const scaleStyle = { transform: `scale(${size})`, transformOrigin: 'left center' };
  ...
  return (
    <div style={scaleStyle} className={className}>
      <div className={folderClassName} style={folderStyle} onClick={handleFolderClick}>
        <div className="folder__back">
          {papers.map((item, i) => (
            <div
              ...
              onKeyDown={open ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onPaperClick?.(i); } } : undefined}
              style={
                open
                  ? {
        ...
        {label && (
          <div className="folder__label" aria-hidden="true">
            {label}
          </div>
        )}
      </div>
